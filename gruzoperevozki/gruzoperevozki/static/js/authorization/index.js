...
formValidation = {
	...
	submitFormHandler: function(e){
		...
		if(this.validatedFields.firstName && this.validatedFields.lastName && this.validatedFields.email && this.validatedFields.password){
			setTimeout((function(){
			this.$submitButton.addClass('form_submit')
				this.$submitButton.removeClass('loading').addClass('success').html('Добро пожаловать, '+this.$firstName.val())
			}).bind(this), 1500);
		}else{
			this.$submitButton.val('Неверное значение!');
			setTimeout((function(){
				if(this.$submitButton.val() == 'Неверное значение!'){
					this.$submitButton.val('Зарегистрироваться');
				}
			}).bind(this), 3000)
			break
		}
	...
	validateText: function($input){
		...
		if($input.val() != ''){
			...
			$input.parent().find('span.label-text').append(' <small class="error">(Пустое поле)</small>');
			return false;
		}
	},
    ...
    if(regEx.test($input.val())){
			...
			$input.parent().addClass('invalid');
			$input.parent().find('span.label-text').append(' <small class="error">(Неверный E-mail)</small>');
			return false;
		}
	},
	validatePassword: function($input){
		...
		if($input.val().length >= 8){
			...
			$input.parent().addClass('invalid');
			$input.parent().find('span.label-text').append(' <small class="error">(Пароль - не менее 7 символов)</small>');
			return false;
		}
	}
}.init();